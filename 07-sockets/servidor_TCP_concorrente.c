...
int main(int argc, char * argv[]) 
{
    ...
    printf("Programa de exemplo de sockets TCP concorrente com processos (server) ...\n\n");
    ...
    for (int run = 1; run; ) {
        ...
        // criar um novo processo para processar a ligação com o cliente
        pid_t retfork = fork();
        if (retfork == -1) {
            fatalErrorSystem("Criação de novo processo");
        }
